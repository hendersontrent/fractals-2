...
// using priors from Fractals 1.0 data
//
...
model {
  ...
  // Log-likelihood
   ...
   y ~ normal(alpha + beta_1*state + beta_2*condition + beta_3*minute + beta_4*inter_1 + beta_5*inter_2 + beta_6*inter_3 + beta_7*inter_4, sigma);
}
...
generated quantities {
  ...
  for (n in 1:N) {
    real y_hat = alpha + beta_1*state[n] + beta_2*condition[n] + beta_3*minute[n] + beta_4*inter_1[n] + beta_5*inter_2[n] + beta_6*inter_3[n] + beta_7*inter_4[n];
